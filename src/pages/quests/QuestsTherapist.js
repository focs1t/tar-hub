...
export default function QuestsTherapist() {
  const quests = questsData.quests.filter(quest => quest.trader === "therapist");
  return (
    <main>
      <style>
        {`
        ...
        <div className="articles">
          {quests.map((quest) => (
            <article key={quest.id} className="article">
              <Link to={`/quests/therapist/${quest.id}`}>
                <img src={quest.image} alt={quest.content} className="article-image" />
              </Link>
              <Link to={`/quests/therapist/${quest.id}`} className="article-title">
                {quest.title}
              </Link>
              <div className="article-text">
                {quest.content}
              </div>
              <Link to={`/quests/therapist/${quest.id}`} className="article-more">
                Читать далее
              </Link>
              {/* Тэги и другой контент */}
            </article>
          ))}
        </div>
        <div className="articles">
