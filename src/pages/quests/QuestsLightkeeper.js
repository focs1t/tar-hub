...
export default function QuestsLightkeeper() {
  const quests = questsData.quests.filter(quest => quest.trader === "lightkeeper");
  return (
      <main>
        <style>
          {`
        ...
        <div className="main-container">
          ...
          {quests.map((quest) => (
            <article key={quest.id} className="article">
              <Link to={`/quests/lightkeeper/${quest.id}`}>
                <img src={quest.image} alt={quest.content} className="article-image" />
              </Link>
              <Link to={`/quests/lightkeeper/${quest.id}`} className="article-title">
              {quest.title}
              </Link>
              <div className="article-text">
                {quest.content}
              </div>
              <Link to={`/quests/lightkeeper/${quest.id}`} className="article-more">
                Читать далее
              </Link>
            </article>
          ))}
